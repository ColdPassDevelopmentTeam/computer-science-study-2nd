...
JDK 21의 `javac`는 source code의 구조와 type을 검사해 bytecode를 만들고, HotSpot JVM은 그
bytecode를 검증하고 class를 준비한 뒤 interpreter와 JIT compiler를 이용해 실행합니다.
...
4. Interpreter가 bytecode를 한 명령씩 실행하며 method 호출에는 stack frame, 객체 생성에는
   주로 heap을 사용합니다.
5. HotSpot JVM은 자주 실행되는 code와 runtime 정보를 바탕으로 JIT compilation을 수행해
   native code로 실행할 수 있습니다.
...
3. Bytecode
4. Class loading
5. HotSpot interpreter와 JIT compiler
...
- JDK 21 HotSpot을 기준으로 interpreter 실행과 runtime 정보를 이용한 JIT compilation의 차이를
  설명합니다.
...
2. JDK 21 HotSpot이 bytecode interpreter와 JIT compiler를 함께 사용하는 이유는 무엇입니까?
